...
SYSTEM_PROMPT = (
    ...
    "- `next` must be one of: \"saas_finder\", \"market\", \"research\", or \"FINISH\". It doesn't include any other text only and only these words\n"
    "- `reason` should be a short (1-2 sentence) justification for the routing decision.\n"
        ...
        "- You will make the requirements/tasks for each worker at once and give them all at once they will return the response and then we will combine all the results from each worker and create a synthesize report \n"
